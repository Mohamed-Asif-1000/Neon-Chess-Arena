...
// New reference to the board wrapper for rotation
...
function updateBoardOrientation() {
  ...
  if (mode === "human") {
    // ROTATE: Black's turn (b) adds the class, White's turn (w) removes it
    if (turn === "b") {
    ...
    // Fixed perspective in computer mode (based on playerColor)
    if (playerColor === "b") {
      boardWrapperEl.classList.add(ROTATED_CLASS);
    } else {
      boardWrapperEl.classList.remove(ROTATED_CLASS);
...
function onSquareClick(r,c){
  ...
  if(selected){
    ...
    if(move){
      ...
      // ROTATION LOGIC: Called after turn switch
      updateBoardOrientation();
